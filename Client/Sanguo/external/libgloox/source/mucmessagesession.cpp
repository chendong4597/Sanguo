...
//     decorate( m );
  ...
  void MUCMessageSession::send(const std::string& message, const StanzaExtensionList& seList)
  {
	  Message m(Message::Groupchat, m_target, message);
	  ...
	  if (seList.size())
	  {
		  StanzaExtensionList::const_iterator it = seList.begin();
		  for (; it != seList.end(); ++it)
			  m.addExtension((*it));
	  }
