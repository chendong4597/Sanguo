                    ...
                    case CDDS_ITEMPREPAINT:
                        {
                            ...
                            wxTreeViewItem tvItem((void *)nmcd.dwItemSpec,
                                                  TVIF_STATE | TVIF_TEXT, TVIS_DROPHILITED);
							const int BUFF_SIZE = 256;
							TCHAR szText[BUFF_SIZE];
							tvItem.pszText = szText;
							tvItem.cchTextMax = BUFF_SIZE;
                            DoGetItem(&tvItem);

                            const UINT tvItemState = tvItem.state;
							if ((tvItemState & TVIS_DROPHILITED) && m_dragStarted)
							{
//								lptvcd->clrTextBk = wxColourToRGB(*wxWHITE);
//								SetItemDropHighlight(TreeView_GetDropHilight(GetHwnd()), false);
//								TreeView_SetItemState(GetHwnd(), nmcd.dwItemSpec, 0, LVIS_DROPHILITED);
//								DrawText(nmcd.hdc, tvItem.pszText, std::wcslen(tvItem.pszText), &nmcd.rc, DT_CENTER | DT_VCENTER);
								wxPoint mousePos = wxGetMousePosition();
								mousePos = ScreenToClient(mousePos);
								wxRect rect;
								if (!GetBoundingRect(TreeView_GetDropHilight(GetHwnd()), rect, true)) { break; }
								wxRect rectDeflated{ rect };
								rectDeflated.Deflate(0, rectDeflated.GetHeight() / 4);	// deflate rect by 50% in vertical direction

								if (mousePos.y < rectDeflated.GetY()) // put src item above dest item
								{
									lptvcd->clrText = wxColourToRGB(*wxCYAN);
									wxDrawLine(nmcd.hdc, rect.GetLeft(), rect.GetTop(), rect.GetRight(), rect.GetTop());
								}
								else if (mousePos.y > rectDeflated.GetY() + rectDeflated.GetHeight()) // put src item below dest item
								{
									lptvcd->clrText = wxColourToRGB(*wxBLUE);
									wxDrawLine(nmcd.hdc, rect.GetLeft(), rect.GetBottom() - 1, rect.GetRight(), rect.GetBottom() - 1);
								}
								else // put src item under dest item (as its child)
								{
									lptvcd->clrText = wxColourToRGB(*wxRED);
									wxDrawLine(nmcd.hdc, rect.GetLeft(), rect.GetBottom() - 1, rect.GetRight(), rect.GetBottom() - 1);
									wxDrawLine(nmcd.hdc, rect.GetLeft(), rect.GetTop(), rect.GetRight(), rect.GetTop());
									wxDrawLine(nmcd.hdc, rect.GetLeft(), rect.GetTop(), rect.GetLeft(), rect.GetBottom());
									wxDrawLine(nmcd.hdc, rect.GetRight(), rect.GetTop(), rect.GetRight(), rect.GetBottom());
								}
					...
					{
						// Draw additional information next to item
						lptvcd->clrTextBk = wxColourToRGB(*wxGREEN);
						*result = CDRF_DODEFAULT;
						break;
					}
