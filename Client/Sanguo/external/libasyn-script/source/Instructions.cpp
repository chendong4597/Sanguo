...
asys::BreakPoint& asys::BreakPoint::operator>>= (const VariableList& variableList)
{
	...
	for (size_t i = 0; i < variableList.getLength(); ++i)
	{
		...
		//do not use CC in the right side of CALL statements, because a variable generated by CC means to be a input variable(like right value in C++).
		assert(variableType != VariableType::input);
