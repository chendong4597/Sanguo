...
// 锟斤拷锟斤拷锟斤拷锟?thumbnail 锟斤拷 锟斤拷锟斤拷锟? 锟斤拷锟斤拷锟斤拷 thumb_format 锟?锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷 锟?锟斤拷锟斤拷锟斤拷锟斤拷
...
void LibRaw::x3f_dpq_interpolate_af(int xstep, int ystep, int scale)
{
	...
	for(int y = 0;  y < imgdata.rawdata.sizes.height+imgdata.rawdata.sizes.top_margin; y+=ystep)
	{
		...
		uint16_t* row0 = &image[imgdata.sizes.raw_width*3*y]; // 袧邪褕邪 褋褌褉芯泻邪
		uint16_t* row_minus = &image[imgdata.sizes.raw_width*3*(y-scale)]; // 小褌褉芯泻邪 胁褘褕械
		uint16_t* row_plus = &image[imgdata.sizes.raw_width*3*(y+scale)]; // 小褌褉芯泻邪 薪懈卸械
