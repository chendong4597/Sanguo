			...
			for (int d = 0; d < 2; d++) {
				...
				for (int k = 0; k < 4; k++)
					if (ydiff[d][k] <= yeps && uvdiff[d][k] <= uveps) {
						homo[d][moff + hvdir[k]]++;
						if (k / 2 == d) {
							// 械褋谢懈 胁 褋芯薪邪锌褉邪胁谢械薪薪芯屑 薪邪锌褉邪胁谢械薪懈懈 懈薪褌械锌芯谢褟褑懈懈 褋谢械写褍褞褖懈械 褌芯褔泻懈 褌邪泻 卸械 谐芯屑芯谐械薪薪褘, 褍褔褌褢屑 懈褏 褌芯卸械
							for (int m = 2; m < 4; ++m) {
								int hvd = m * hvdir[k];
...
void AAHD::make_ahd_rb_hv(int i) {
	...
	int js = libraw.COLOR(i, 0) & 1;
	int kc = libraw.COLOR(i, js);
	js ^= 1; // 薪邪褔邪谢褜薪邪褟 泻芯芯褉写懈薪邪褌邪 蟹械谢褢薪芯谐芯
	int hvdir[2] = {
		Pe,
		Ps };
	// 懈薪褌械褉锌芯谢褟褑懈褟 胁械褉褌懈泻邪谢褜薪褘褏 胁械褉褌懈泻邪谢褜薪芯 懈 谐芯褉懈蟹芯薪褌邪谢褜薪褘褏 谐芯褉懈蟹芯薪褌邪谢褜薪芯
	for (int j = js; j < iwidth; j += 2) {
		...
		for (int d = 0; d < 2; ++d) {
			ushort3 *cnr;
			cnr = &rgb_ahd[d][moff];
			int c = kc ^ (d << 1); // 褑胁械褌 褋芯芯褌胁械褌褋胁械薪薪芯谐芯 薪邪锌褉邪胁谢械薪懈褟, 写谢褟 谐芯褉懈蟹芯薪褌邪谢褜薪芯谐芯 c = kc, 写谢褟 胁械褉褌懈泻邪谢褜薪芯谐芯 c=kc^2
...
void AAHD::make_ahd_rb_last(int i) {
	...
	for (int j = 0; j < iwidth; j++) {
		for (int d = 0; d < 2; ++d) {
			...
			int c = kc ^ 2;
			if ((j & 1) != js) {
				// 褌芯褔泻邪 蟹械谢褢薪芯谐芯, 写谢褟 胁械褉褌懈泻邪谢褜薪芯谐芯 薪邪锌褉邪胁谢械薪懈褟 薪褍卸械薪 邪谢褜褌械褉薪邪褌懈胁薪褘泄 褋褌褉芯褔薪芯屑褍 褑胁械褌
				c ^= d << 1;
			}
			int bh, bk;
			int bgd = 0;
			for (int k = 0; k < 3; ++k)
				for (int h = 0; h < 3; ++h) {
					// 谐褉邪写懈械薪褌 蟹械谢褢薪芯谐芯 锌谢褞褋 谐褉邪写懈械薪褌 {r,b}
