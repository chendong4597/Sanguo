...
#include "../utils/idList.h"
#include "../utils/idStaticList.h"
#include "../LogUtil.h"
#include "../TimeUtil.h"
...
const char * jobNames[] = {
	ASSERT_ENUM_STRING(JOBLIST_ANIMATION,	0 ),
...
struct threadStats_t {
	...
	UI64			endTime;
	UI64			waitTime;
	UI64			threadExecTime[MAX_THREADS];
	UI64			threadTotalTime[MAX_THREADS];
};
...
public:
	...
	//------------------------
	FORCEINLINE void		AddJob( jobRun_t function, void * data );
	FORCEINLINE void		InsertSyncPoint( jobSyncType_t syncType );
	...
	UI64					GetSubmitTimeMicroSec() const { return threadStats.submitTime; }
	UI64					GetStartTimeMicroSec() const { return threadStats.startTime; }
	UI64					GetFinishTimeMicroSec() const { return threadStats.endTime; }
	UI64					GetWaitTimeMicroSec() const { return threadStats.waitTime; }
	UI64					GetTotalProcessingTimeMicroSec() const;
	UI64					GetTotalWastedTimeMicroSec() const;
	UI64					GetUnitProcessingTimeMicroSec( int unit ) const;
	UI64					GetUnitWastedTimeMicroSec( int unit ) const;
...
========================
*/
FORCEINLINE void idParallelJobList_Threads::AddJob( jobRun_t function, void * data ) {
	assert( done );
#if defined( _DEBUG )
	...
	if ( 1 ) { // JDC: this never worked in tech5!  !jobList.IsFull() ) {
		...
		for ( int i = 0; i < numRegisteredJobs; ++i ) {
			if ( currentJobCount[ i ] > 0 ) {
				LOGI("Job: {0}, #{1}", registeredJobs[i].name, currentJobCount[i]);
			}
		}
		LOGE("Can't add job: '{0}', too many jobs {1}", GetJobName(function), jobList.Num());
	}
}
...
========================
*/
FORCEINLINE void idParallelJobList_Threads::InsertSyncPoint( jobSyncType_t syncType ) {
	assert( done );
	switch( syncType ) {
		case SYNC_SIGNAL: {
			assert( !hasSignal );
...
void idParallelJobList_Threads::Submit( idParallelJobList_Threads * waitForJobList, int parallelism ) {
	...
	deferredThreadStats.submitTime = TimeUtil::getTimeMicroseconds();
...
void idParallelJobList_Threads::Wait() {
	if ( jobList.Num() > 0 ) {
		...
		bool waited = false;
		UI64 waitStart = TimeUtil::getTimeMicroseconds();
		...
		UI64 waitEnd = TimeUtil::getTimeMicroseconds();
...
========================
*/
UI64 idParallelJobList_Threads::GetTotalProcessingTimeMicroSec() const {
	UI64 total = 0;
...
========================
*/
UI64 idParallelJobList_Threads::GetTotalWastedTimeMicroSec() const {
	UI64 total = 0;
	for ( int unit = 0; unit < MAX_THREADS; unit++ ) {
...
========================
*/
UI64 idParallelJobList_Threads::GetUnitProcessingTimeMicroSec( int unit ) const {
...
========================
*/
UI64 idParallelJobList_Threads::GetUnitWastedTimeMicroSec( int unit ) const {
	if ( unit < 0 || unit >= MAX_THREADS ) {
		return 0;
	}
...
int idParallelJobList_Threads::RunJobsInternal( unsigned int threadNum, threadJobListState_t & state, bool singleJob ) {
	...
	if ( deferredThreadStats.startTime == 0 ) {
		deferredThreadStats.startTime = TimeUtil::getTimeMicroseconds();	// first time any thread is running jobs from this list
	}
...
//                  LOGD("{0:1.1f} milliseconds for a single '{1}' job from job list {2} on thread {3}", longJobTime, jobName, jobListName, threadNum);
// 				}
// 			}
// #endif
		}
		...
		if ( signalJobCount[state.signalIndex].Decrement() == 0 ) {
			...
			if ( state.signalIndex == signalJobCount.Num() - 1 ) {
				deferredThreadStats.endTime = TimeUtil::getTimeMicroseconds();
				return ( result | RUN_DONE );
			}
		}
...
int idParallelJobList_Threads::RunJobs( unsigned int threadNum, threadJobListState_t & state, bool singleJob ) {
	UI64 start = TimeUtil::getTimeMicroseconds();
	...
	deferredThreadStats.threadTotalTime[threadNum] += TimeUtil::getTimeMicroseconds() - start;
...
========================
*/
UI64 idParallelJobList::GetSubmitTimeMicroSec() const {
...
========================
*/
UI64 idParallelJobList::GetStartTimeMicroSec() const {
...
========================
*/
UI64 idParallelJobList::GetFinishTimeMicroSec() const {
...
========================
*/
UI64 idParallelJobList::GetWaitTimeMicroSec() const {
...
========================
*/
UI64 idParallelJobList::GetTotalProcessingTimeMicroSec() const {
...
========================
*/
UI64 idParallelJobList::GetTotalWastedTimeMicroSec() const {
...
========================
*/
UI64 idParallelJobList::GetUnitProcessingTimeMicroSec( int unit ) const {
...
========================
*/
UI64 idParallelJobList::GetUnitWastedTimeMicroSec( int unit ) const {
...
void idJobThread::Start( core_t core, unsigned int threadNum ) {
	...
	tstring threadName = fmt::format("JobListProcessor{0}", threadNum);
	StartWorkerThread(threadName.c_str(), core, THREAD_NORMAL, JOB_THREAD_STACK_SIZE );
}
