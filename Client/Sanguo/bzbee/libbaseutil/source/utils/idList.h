...
#include "idSort.h"
#include "../BaseType.h"
#include "../allocator/AllocCommon.h"
...
FORCEINLINE void * idListArrayNew(int num, bool zeroBuffer) {
	_type_ * ptr = NULL;
	if (zeroBuffer) {
...
FORCEINLINE void idListArrayDelete(void *ptr, int num) {
...
*/
template< typename _type_, memTag_t _tag_ >
FORCEINLINE void * idListArrayResize(void * voldptr, int oldNum, int newNum, bool zeroBuffer) {
	_type_ * oldptr = (_type_ *)voldptr;
	_type_ * newptr = NULL;
	if (newNum > 0) {
		newptr = (_type_ *)idListArrayNew<_type_, _tag_>(newNum, zeroBuffer);
		int overlap = std::min(oldNum, newNum);
...
*/
template< class type >
FORCEINLINE type *idListNewElement(void) {
	return new type;
}
...
//	void			Sort( cmp_t *compare = ( cmp_t * )&idListSortCompare<_type_, _tag_> );
	...
	void			SetMemTag(memTag_t tag_) { memTag = (uint8)tag_; };
...
*/
template< typename _type_, memTag_t _tag_ >
FORCEINLINE idList<_type_, _tag_>::idList(int newgranularity) {
...
*/
template< typename _type_, memTag_t _tag_ >
FORCEINLINE idList<_type_, _tag_>::idList(const idList &other) {
	list = NULL;
	*this = other;
}
...
*/
template< typename _type_, memTag_t _tag_ >
FORCEINLINE idList<_type_, _tag_>::~idList() {
	Clear();
}
...
*/
template< typename _type_, memTag_t _tag_ >
FORCEINLINE void idList<_type_, _tag_>::Clear() {
	if (list) {
...
*/
template< typename _type_, memTag_t _tag_ >
FORCEINLINE void idList<_type_, _tag_>::DeleteContents(bool clear) {
	int i;
...
*/
template< typename _type_, memTag_t _tag_ >
FORCEINLINE size_t idList<_type_, _tag_>::Allocated() const {
...
*/
template< typename _type_, memTag_t _tag_ >
FORCEINLINE size_t idList<_type_, _tag_>::Size() const {
...
*/
template< typename _type_, memTag_t _tag_ >
FORCEINLINE size_t idList<_type_, _tag_>::MemoryUsed() const {
...
*/
template< typename _type_, memTag_t _tag_ >
FORCEINLINE int idList<_type_, _tag_>::Num() const {
	return num;
}
...
*/
template< typename _type_, memTag_t _tag_ >
FORCEINLINE int idList<_type_, _tag_>::NumAllocated() const {
	return size;
}
...
*/
template< typename _type_, memTag_t _tag_ >
FORCEINLINE void idList<_type_, _tag_>::SetNum(int newnum) {
	assert(newnum >= 0);
	if (newnum > size) {
		Resize(newnum);
	}
...
*/
template< typename _type_, memTag_t _tag_ >
FORCEINLINE void idList<_type_, _tag_>::SetGranularity(int newgranularity) {
	int newsize;
...
*/
template< typename _type_, memTag_t _tag_ >
FORCEINLINE int idList<_type_, _tag_>::GetGranularity() const {
	return granularity;
}
...
*/
template< typename _type_, memTag_t _tag_ >
FORCEINLINE void idList<_type_, _tag_>::Condense() {
	if (list) {
		if (num) {
			Resize(num);
		}
...
*/
template< typename _type_, memTag_t _tag_ >
FORCEINLINE void idList<_type_, _tag_>::Resize(int newsize) {
	assert(newsize >= 0);
...
*/
template< typename _type_, memTag_t _tag_ >
FORCEINLINE void idList<_type_, _tag_>::Resize(int newsize, int newgranularity) {
	assert(newsize >= 0);
...
*/
template< typename _type_, memTag_t _tag_ >
FORCEINLINE void idList<_type_, _tag_>::AssureSize(int newSize) {
	int newNum = newSize;
...
*/
template< typename _type_, memTag_t _tag_ >
FORCEINLINE void idList<_type_, _tag_>::AssureSize(int newSize, const _type_ &initValue) {
	int newNum = newSize;
...
*/
template< typename _type_, memTag_t _tag_ >
FORCEINLINE void idList<_type_, _tag_>::AssureSizeAlloc(int newSize, new_t *allocator) {
	int newNum = newSize;
...
*/
template< typename _type_, memTag_t _tag_ >
FORCEINLINE idList<_type_, _tag_> & idList<_type_, _tag_>::operator=(const idList<_type_, _tag_> &other) {
	int	i;
...
*/
template< typename _type_, memTag_t _tag_ >
FORCEINLINE const _type_ & idList<_type_, _tag_>::operator[](int index) const {
	assert(index >= 0);
	assert(index < num);
...
*/
template< typename _type_, memTag_t _tag_ >
FORCEINLINE _type_ & idList<_type_, _tag_>::operator[](int index) {
	assert(index >= 0);
	assert(index < num);
...
*/
template< typename _type_, memTag_t _tag_ >
FORCEINLINE _type_ * idList<_type_, _tag_>::Ptr() {
	return list;
}
...
*/
template< typename _type_, memTag_t _tag_ >
const FORCEINLINE _type_ * idList<_type_, _tag_>::Ptr() const {
	return list;
}
...
*/
template< typename _type_, memTag_t _tag_ >
FORCEINLINE _type_ & idList<_type_, _tag_>::Alloc() {
	if (!list) {
		Resize(granularity);
	}
...
*/
template< typename _type_, memTag_t _tag_ >
FORCEINLINE int idList<_type_, _tag_>::Append(_type_ const & obj) {
	if (!list) {
		Resize(granularity);
	}
...
*/
template< typename _type_, memTag_t _tag_ >
FORCEINLINE int idList<_type_, _tag_>::Insert(_type_ const & obj, int index) {
	if (!list) {
		Resize(granularity);
	}
...
*/
template< typename _type_, memTag_t _tag_ >
FORCEINLINE int idList<_type_, _tag_>::Append(const idList< _type_, _tag_ > &other) {
	if (!list) {
		if (granularity == 0) {	// this is a hack to fix our memset classes
			granularity = 16;
		}
...
*/
template< typename _type_, memTag_t _tag_ >
FORCEINLINE int idList<_type_, _tag_>::AddUnique(_type_ const & obj) {
	int index;
...
*/
template< typename _type_, memTag_t _tag_ >
FORCEINLINE int idList<_type_, _tag_>::FindIndex(_type_ const & obj) const {
	int i;
...
*/
template< typename _type_, memTag_t _tag_ >
FORCEINLINE _type_ *idList<_type_, _tag_>::Find(_type_ const & obj) const {
	int i;
...
*/
template< typename _type_, memTag_t _tag_ >
FORCEINLINE int idList<_type_, _tag_>::FindNull() const {
	int i;
...
*/
template< typename _type_, memTag_t _tag_ >
FORCEINLINE int idList<_type_, _tag_>::IndexOf(_type_ const *objptr) const {
	int index;
...
*/
template< typename _type_, memTag_t _tag_ >
FORCEINLINE bool idList<_type_, _tag_>::RemoveIndex(int index) {
	int i;
...
*/
template< typename _type_, memTag_t _tag_ >
FORCEINLINE bool idList<_type_, _tag_>::RemoveIndexFast(int index) {
...
*/
template< typename _type_, memTag_t _tag_ >
FORCEINLINE bool idList<_type_, _tag_>::Remove(_type_ const & obj) {
	int index;
...
//*/
//template< typename _type_, memTag_t _tag_ >
//FORCEINLINE void idList<_type_,_tag_>::Sort( cmp_t *compare ) {
//	if ( !list ) {
//		return;
//	}
...
*/
template< typename _type_, memTag_t _tag_ >
FORCEINLINE void idList<_type_, _tag_>::SortWithTemplate(const idSort<_type_> & sort) {
	if (list == NULL) {
		return;
	}
	sort.Sort(Ptr(), Num());
...
//*/
//template< typename _type_, memTag_t _tag_ >
//FORCEINLINE void idList<_type_,_tag_>::SortSubSection( int startIndex, int endIndex, cmp_t *compare ) {
//	if ( !list ) {
//		return;
//	}
//	if ( startIndex < 0 ) {
