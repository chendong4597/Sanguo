...
namespace LPCD {
	...
	template<> struct Type<LuaPlus::LuaState*> {
		static void Push(lua_State* L, LuaPlus::LuaState* value)						{  lua_pushlightuserdata(L, value);  }
		static bool Match(lua_State* L, int idx)										{  (void)L, (void)idx;  return true;  }
		static LuaPlus::LuaState* Get(lua_State* L, int idx)							{  return lua_State_to_LuaState(L);  }
	};
} // namespace LPCD
