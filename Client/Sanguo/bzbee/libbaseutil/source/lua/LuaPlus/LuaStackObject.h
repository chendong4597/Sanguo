...
namespace LPCD {
	...
	template<> struct Type<LuaStackObject> {
		static void Push(lua_State* L, const LuaStackObject& value)						{  lua_pushvalue(L, value.m_stackIndex);  }
		static bool Match(lua_State* L, int idx)											{  (void)L, (void)idx;  return true;  }
		static LuaStackObject Get(lua_State* L, int idx)									{  return LuaStackObject(lua_State_to_LuaState(L), idx);  }
	};
