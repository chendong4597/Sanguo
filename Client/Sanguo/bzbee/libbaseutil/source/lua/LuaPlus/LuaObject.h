...
/*	template <typename T>
	...
	**/
	...
	LuaObject& AssignCFunctionDirect(LuaState* state, Func func, unsigned int nupvalues = 0) {
	...
	LuaObject& AssignCFunctionDirect(LuaState* state, const Callee& callee, Func func, unsigned int nupvalues = 0) {
		const void* pCallee = &callee;
	...
	template <typename Callee, typename Func>
	LuaObject& AssignCFunctionObjectDirect(LuaState* state, const Callee* callee, Func func, unsigned int nupvalues = 0) {
	...
	template <class Callee>
	LuaObject& Register(const char* funcName, const Callee& callee, int (Callee::*func)(LuaState*), int nupvalues = 0) {
		const void* pCallee = &callee;
	...
	template <class Callee>
	LuaObject& RegisterObjectFunctor(const char* funcName, int (Callee::*func)(LuaState*), int nupvalues = 0) {
	...
	template <typename Func>
	LuaObject& RegisterDirect(const char* funcName, Func func, unsigned int nupvalues = 0) {
	...
	template <typename Callee, typename Func>
	LuaObject& RegisterDirect(const char* funcName, const Callee& callee, Func func, unsigned int nupvalues = 0) {
		const void* pCallee = &callee;
	...
	template <typename Callee, typename Func>
	LuaObject& RegisterObjectDirect(const char* funcName, const Callee* callee, Func func, unsigned int nupvalues = 0) {
	...
	template <typename Callee, typename Func>
	LuaObject& RegisterInPlaceObjectDirect(const char* funcName, const Callee* callee, Func func, unsigned int nupvalues = 0) {
...
namespace LPCD {
	...
	template<> struct Type<LuaObject> {
		static void Push(lua_State* L, const LuaObject& value)						{  value.Push(L);  }
		static bool Match(lua_State* L, int idx)										{  (void)L, (void)idx;  return true;  }
		static LuaPlus::LuaObject Get(lua_State* L, int idx)							{  return LuaObject(lua_State_to_LuaState(L), idx);  }
	};
	...
	void PropertyCreate(const LuaPlus::LuaObject& metatableObj, const char* varName, VarType Object::* var, bool read = true, bool write = true) {
    ...
    void Metatable_IntegratePropertySupport(LuaPlus::LuaObject& metatableObj, bool inPlace = false);
...
template <typename T>
LuaObject& LuaObject::SetNil(const T& key) {
	luaplus_assert(L);
	LUA_FASTREF_PUSH();
...
template <typename ValueT>
LuaObject& LuaObject::Assign(lua_State* ls, const ValueT& value) {
	if (L)
		lua_fastunref(L, ref);
	L = ls;
	LPCD::Type<ValueT>::Push(L, value);
...
template <typename ValueT>
LuaObject& LuaObject::Assign(lua_State* ls, const ValueT& value, int len) {
	if (L)
		lua_fastunref(L, ref);
	L = ls;
	LPCD::Push(L, value, len);
