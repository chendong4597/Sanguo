...
namespace LPCD
{
	...
	template<> struct Type<int (*)(LuaState*)> {
		...
		static bool Match(lua_State* L, int idx)									{  return lua_type(L, idx) == LUA_TFUNCTION;  }
		static void* Get(lua_State* L, int /*idx*/)								{  return lua_State_to_LuaState(L);  }
	};
	...
	public:
		static int LuaStateMemberDispatcher(lua_State* L) {
	...
	public:
		static int Object_MemberDispatcher_to_LuaState(lua_State* L) {
			typedef int (Callee::*Functor)(LuaPlus::LuaState*);
