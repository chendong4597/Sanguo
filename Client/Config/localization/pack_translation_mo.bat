...
:: 复制文件并重命名
copy i18n\i18n.xml all\all_key.xml

@echo copy i18n.xml to all_key.xml
...
::临时资源目录
set TEMP_DIR=%ROOT_DIR%\ThirdConfig\localization\all
...
:: 项目资源目录
set RESOURCE_DIR=%ROOT_DIR%\ThirdConfig\localization\bin
...
:: 工程配置目录
set PROJECT_DIR=%ROOT_DIR%\ThirdReich\Content\config
...
%TOOL_DIR%\I18NConv.exe --merge -i %LOCALIZATION_ROOT_DIR%\extra\empty_%LANGUAGE%.xml -p CR_PREFER_NEW -o %I18N_DIR%\i18n.xml
...
%TOOL_DIR%\I18NConv.exe --tomo -i %I18N_DIR%\i18n.xml -k %TEMP_DIR%\all_key.xml -l %LANGUAGE% -o %RESOURCE_DIR%\config\%LANGUAGE%.mo > %TEMP_DIR%\pack_translation_warning.xml
if %errorlevel% NEQ 0 goto error_exit

@echo copying project files...
copy %RESOURCE_DIR%\config\%LANGUAGE%.mo %PROJECT_DIR%\
goto end
