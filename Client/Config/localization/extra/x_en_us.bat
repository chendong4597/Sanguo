...
%TOOL_DIR%\I18NConv.exe --extract -i %ROOT_DIR%\i18n.xml -l %BASE_LANGUAGE% -l %LANGUAGE% -p EXTRA_FULL -o %ROOT_DIR%\extra\empty_%LANGUAGE%.xml
...
%TOOL_DIR%\I18NConv.exe --filter -i %ROOT_DIR%\extra\empty_%LANGUAGE%.xml -l %BASE_LANGUAGE% -l %LANGUAGE% -f %ROOT_DIR%\extra\temp_empty.xml -f %ROOT_DIR%\extra\temp_non_empty.xml
